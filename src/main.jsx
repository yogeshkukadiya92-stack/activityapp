...
function ConditionalResults({activity,responses=[]}){
  const answers=responses.map(item=>({...parseConditionalAnswer(item.answer),id:item.id})),followUps=answers.filter(item=>item.followUp.trim());
  return <div className="conditional-results"><PollPreview activity={activity} responses={responses}/>{followUps.length?<section><header><strong>Follow-up responses</strong><span>{followUps.length}</span></header>{followUps.slice(0,6).map(item=><blockquote key={item.id}><b>{item.choice}</b><span>{item.followUp}</span></blockquote>)}</section>:<p>Follow-up responses will appear after participants choose a branch.</p>}</div>;
}
...
function EditorParticipantPreview({activity,options,settings}){
  if(!activity)return null;
  if(activity.type==='Conditional Poll'){const branchIndex=(settings.followUps||[]).findIndex(value=>String(value||'').trim()),branch=branchIndex>=0?branchIndex:0;return <div className="studio-conditional-preview"><div className="studio-phone-options">{options.map((option,index)=><label key={index} className={index===branch?'selected':''}><input type="radio" name="studio-preview-option" checked={index===branch} readOnly/>{option}</label>)}</div>{settings.followUps?.[branch]&&<section><small>If “{options[branch]}”</small><strong>{settings.followUps[branch]}</strong><textarea disabled placeholder="Participant follow-up answer"/></section>}</div>}
  ...
  if(activity.type==='Number Count')return <input className="studio-phone-number" type="number" min={settings.min} max={settings.max} placeholder="Enter a number"/>;
...
function ActivityEditor({workshopId,onClose,onChanged,user,onLogout,onJoin,onNavigate,onPresent}){
  ...
  const current=workshop?.activities?.[selected],options=current?defaultOptionsFor(current):[],settings={allowMultiple:current?.type==='Multiple Answers',showResults:true,anonymous:false,min:1,max:10,leftLabel:'Not at all',rightLabel:'Absolutely',imageUrl:'',optionImages:[],followUps:[],correctOption:0,...(current?.settings||{})};
  ...
  const removeOption=index=>update({options:options.filter((_,itemIndex)=>itemIndex!==index),settings:{...settings,optionImages:(settings.optionImages||[]).filter((_,itemIndex)=>itemIndex!==index),followUps:(settings.followUps||[]).filter((_,itemIndex)=>itemIndex!==index)}});
  ...
  return <div className="studio-shell"><SideNav active="workshops" onOpenParticipant={onJoin} onNavigate={onNavigate}/><main className="studio-main">
    ...
    <div className="studio-workspace"><aside className="studio-rail"><header><h2>Activities</h2><span>{workshop.activities.length}</span></header><div>{workshop.activities.map((item,index)=>{const [icon,color]=typeMeta(item.type);return <button key={item.id} className={selected===index?'selected':''} onClick={()=>setSelected(index)}><b>{index+1}</b><i className={`activity-icon ${color}`}><Icon name={icon}/></i><span><strong>{item.title}</strong><small>{item.type}</small></span></button>})}</div><button className="studio-add" onClick={add}><Icon name="plus"/> Add activity</button></aside>
      <section className="studio-editor"><header><input aria-label="Activity title" value={current?.title||''} onChange={event=>update({title:event.target.value})}/><label><Icon name={previewIcon}/><select aria-label="Activity type" value={current?.type||'Live Poll'} onChange={event=>{const type=event.target.value;update({type,options:optionTypes.has(type)?defaultOptionsFor({type,title:current.title}):[],settings:{...settings,allowMultiple:type==='Multiple Answers'}})}}>{activityTypes.map(type=><option key={type}>{type}</option>)}</select></label></header><div className="studio-scroll"><label className="studio-label">QUESTION</label><textarea className="studio-question" maxLength="280" value={current?.question||''} onChange={event=>update({question:event.target.value})}/><small className="studio-count">{current?.question?.length||0}/280</small>
        {optionTypes.has(current?.type)&&<section className="studio-options"><h3>{current.type==='Arrange the Steps'?'STEPS IN THE CORRECT ORDER':'ANSWER OPTIONS'}</h3>{options.map((option,index)=><div key={index}><span>⠿</span><input aria-label={`Option ${index+1}`} value={option} onChange={event=>setOption(index,event.target.value)}/><button aria-label={`Delete option ${index+1}`} onClick={()=>removeOption(index)}>×</button></div>)}<button className="studio-add-option" onClick={()=>update({options:[...options,current.type==='Arrange the Steps'?`Step ${options.length+1}`:`Option ${options.length+1}`],settings:{...settings,followUps:[...(settings.followUps||[]),'']}})}><Icon name="plus"/> Add {current.type==='Arrange the Steps'?'step':'option'}</button></section>}
        {current?.type==='Conditional Poll'&&<section className="studio-conditionals"><h3>BRANCH FOLLOW-UPS</h3><p>Add a question to either, both, or none of the answer branches.</p>{options.map((option,index)=><label key={index}><span>If answer is <b>{option}</b></span><textarea maxLength="180" value={settings.followUps?.[index]||''} onChange={event=>{const followUps=[...(settings.followUps||[])];followUps[index]=event.target.value;setSetting('followUps',followUps)}} placeholder="Optional follow-up question…"/><small>{settings.followUps?.[index]?.length||0}/180</small></label>)}</section>}
...
function ParticipantActivityInput({activity,answer,setAnswer}){
  ...
  if(activity.type==='Conditional Poll'){const response=parseConditionalAnswer(answer),branchIndex=options.indexOf(response.choice),followUpQuestion=branchIndex>=0?String(settings.followUps?.[branchIndex]||'').trim():'';return <div className="participant-conditional"><div className="participant-options">{options.map(option=><label key={option} className={response.choice===option?'selected':''}><input type="radio" name="participant-option" checked={response.choice===option} onChange={()=>setAnswer(conditionalAnswer(option,''))}/>{option}</label>)}</div>{followUpQuestion&&<section><small>Follow-up for “{response.choice}”</small><h2>{followUpQuestion}</h2><textarea autoFocus value={response.followUp} maxLength={120} onChange={event=>setAnswer(conditionalAnswer(response.choice,event.target.value))} placeholder="Type your follow-up answer…"/><div className="char-count">{response.followUp.length}/120</div></section>}</div>}
  ...
  if(activity.type==='Number Count')return <input className="participant-number" type="number" min={settings.min??0} max={settings.max??10} value={answer} onChange={event=>setAnswer(event.target.value)} placeholder="Enter a number"/>;
...
const answerIsComplete=(activity,answer)=>{if(!String(answer||'').trim())return false;if(activity?.type!=='Conditional Poll')return true;const response=parseConditionalAnswer(answer),index=defaultOptionsFor(activity).indexOf(response.choice),followUp=String(activity.settings?.followUps?.[index]||'').trim();return Boolean(response.choice)&&(!followUp||Boolean(response.followUp.trim()))};
